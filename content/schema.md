...
- `learnExperience`: Optional narrative Learn presentation model. `version` identifies the renderer contract; `flowVersion` identifies its saved-progress migration; `flow` documents the reusable scene IDs; and `scenes` supplies presentation copy for arrive, encounter, pattern, rehearse, speak, conversation, culture, and completion. Scene data may configure speakers, artwork descriptions, conversation stages, and completion details without changing the reviewed teaching pack. A culture scene may use a short `lead` for its editorial thought and `body` for the quieter explanation. Practice and review continue to read the reviewed vocabulary, pronunciation, flashcards, quiz, and speaking fields.
- `learnExperience.practiceUnlock`: Optional summary of the reviewed lesson material surfaced in Practice after the episode. `topic` must match an existing Practice topic, `phrases` must be drawn from the lesson’s reviewed language, and `conversationPatterns` is a non-negative count. When `requiresCompletion` is true, the material remains locked until that lesson is complete; existing completion records satisfy the gate without replay. The renderer must not treat this presentation metadata as a separate vocabulary source.
- `learnExperience.completionAchievements`: Optional stable achievement IDs awarded on the lesson’s first completion. Achievement storage remains idempotent across refresh and replay.
- Lessons opt into the shared Lesson 1 narrative renderer with `learnExperience.version: 2`. The scene sequence is defined once in the renderer: arrive, encounter, pattern, rehearse, speak, conversation, culture, then completion. Lessons without this metadata retain their existing flow.
- `learnExperience.scenes.*.byChoice`: Optional shallow overrides keyed by an existing `learnerChoices.options[].id`. They carry the selected branch into pattern examples and speaking. They never write memory themselves.
- `learnExperience.scenes.pattern.moves`: Bilingual `{label, spanish, english}` examples for this lesson's communication pattern. `showReference` exposes the existing canonical vocabulary as optional reference. Keep grammar within the curriculum's single concept, or review only for a capstone.
- `learnExperience.scenes.encounter.speaker` identifies the character who opens the scene. `showModel` makes the conversation builder available as a bilingual model with normal and slow audio. Existing learner choices appear inside the encounter, with the existing explicit choice-memory writes.
- `conversationBuilder`: Ordered rehearsal and speaking turns. Each turn has a stable `id`, `speaker`, bilingual `carlosSpanish`/`carlosEnglish` prompt and `responseSpanish`/`responseEnglish` model, learner-facing `prompt`, `tip`, `feedback`, and `practiceOptions`. The field names remain compatible when the speaker is Ana, Marta, or another character. Exactly one practice option must match the model response. Optional `speakingPrompt` replaces the rehearsal instruction with a personal production task; `byChoice` supplies equivalent branches. Example family and routine details must not be stored as learner facts.
- `learnExperience.scenes.*.image`, `imageAlt`, and `caption` may use existing project artwork for the actual setting. `culture.showDiscovery` keeps the existing optional Living World interaction available; `completion.showCeremony` preserves a chapter ceremony and postcard inside the narrative completion screen. `cta` supplies a concise scene-specific action label.
- Narrative progress defaults to `lessonFlowVersion: 6`; `learnExperience.flowVersion` preserves newer contracts such as Lesson 2’s version 7. Migration preserves completed lessons, XP, rewards, quiz review, and story choices while mapping the former screen to the shared flow. Old exercise attempts are reset because the new prompts differ. Starting a new session records its version before advancing.
