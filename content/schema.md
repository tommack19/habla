...
- `learnExperience`: Optional narrative Learn presentation model. `version` identifies the renderer contract; `flow` orders reusable scene IDs; `scenes` supplies presentation copy for scene types such as arrive, encounter, pattern, rehearse, speak, conversation, culture, and completion. A culture scene may use a short `lead` for its editorial thought and `body` for the quieter explanation. It does not replace the canonical content pack: Practice and review continue to read the reviewed vocabulary, pronunciation, flashcards, quiz, and speaking fields.
- `learnExperience.practiceUnlock`: Optional summary of the reviewed lesson material surfaced in Practice after the episode. `topic` must match an existing Practice topic, `phrases` must be drawn from the lesson’s reviewed language, and `conversationPatterns` is a non-negative count. The renderer must not treat this presentation metadata as a separate vocabulary source.
